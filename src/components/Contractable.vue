<template>
  <CCol class="card bg-light border-top-3 border-top-dark border-secondary p-0 mt-3">
    ...
    <CForm class="p-2">
      <CRow class="mt-3">
        <CCol xs="8" sm="9">
          <CFormInput type="file" ref="fileInput" @change="onFilePicked" id="formFileLg"/>
        </CCol>
        <CCol xs="4" sm="3">
          ...
          <CTableBody>
            <CTableRow v-for="(contract, index) in contracts">
              ...
              <CTableDataCell :aria-colspan="2">
                <button class="btn btn-sm btn-danger text-white m-1" @click="deleteFile(contract.id, index)" v-c-tooltip="{content: 'Delete Contract', placement: 'top'}">
